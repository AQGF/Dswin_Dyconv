...
from models.TransMorph_bspl_DSwin3D_AGDynConv_Lite_Expanded import CONFIGS as CONFIGS_TM
import models.TransMorph_bspl_DSwin3D_AGDynConv_Lite_Expanded as TransMorph_bspl_DSwinAGDynConvLiteExpanded
...
def main():
    ...
    model_idx = -1
    config = CONFIGS_TM['TransMorphBSpline-DSwin3D-AGDynConv-Lite-Expanded']
    config.detach_guidance = True
    config.offset_mag_weight = 0.0
    config.offset_smooth_weight = 0.1
    model_folder = 'TransMorphBSpline_DSwin3D_AGDynConv_Lite_Expanded_e64_rc24_AdamW_MultiNCC3_9_15_detachTrue_offmag_{}_offsmooth_{}/'.format(
        config.offset_mag_weight,
        config.offset_smooth_weight,
    )
    model_dir = 'experiments/' + model_folder
    label_dict = utils.process_label()
    ...
    line = ''
    for i in range(46):
        line = line + ',' + label_dict[i]
    csv_writter(line + ',' + 'non_jec', 'Quantitative_Results/' + model_folder[:-1])

    model = TransMorph_bspl_DSwinAGDynConvLiteExpanded.TranMorphBSplineDSwinAGDynConvLiteExpandedNet(config)
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            line = utils.dice_val_substruct(def_out.long(), y_seg.long(), study_idx)
            line = line + ',' + str(np.sum(jac_det <= 0) / np.prod(target.shape))
            csv_writter(line, 'Quantitative_Results/' + model_folder[:-1])
            eval_det.update(np.sum(jac_det <= 0) / np.prod(target.shape), x.size(0))
            print('det < 0: {}'.format(np.sum(jac_det <= 0) / np.prod(target.shape)))
        ...
        print('Deformed DSC: {:.3f} +- {:.3f}, Affine DSC: {:.3f} +- {:.3f}'.format(eval_dsc_def.avg, eval_dsc_def.std, eval_dsc_raw.avg, eval_dsc_raw.std))
