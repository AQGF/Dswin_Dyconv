...
from models.TransMorph_DSwin3D_AGDynConv import (
    TransMorphDSwin3DAGDynConv,
...
def main():
    ...
    save_dir = 'TransMorph_DSwin3D_AGDynConv_ncc_{}_diffusion_{}_offmag_{}_offsmooth_{}/'.format(
        weights[0],
        weights[1],
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            warped, flow, aux = model(x_in)

            sim_loss = criterions[0](warped, y) * weights[0]
            flow_reg_loss = criterions[1](flow, y) * weights[1]
            off_mag_loss = compute_offset_magnitude_loss(
                aux['attn_aux'],
                penalty=config.offset_reg_penalty,
            ) * config.offset_mag_weight
            off_smooth_loss = compute_offset_smoothness_loss(
                aux['attn_aux'],
                penalty=config.offset_reg_penalty,
            ) * config.offset_smooth_weight
