...
import models.configs_TransMorph_DSwin3D as configs
...
class FeedForward3D(nn.Module):
    def __init__(self, dim, expansion_factor=2.0, bias=True):
        super().__init__()
...
class DeformableSlidingWindowAttention3D(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
    ...
    def forward(self, x):
        ...
        for q_branch, k_branch, v_branch, offset_branch, rel_offsets, bias in zip(
            ...
            branch_outputs.append(
                self._branch_attention(q_branch, k_branch, v_branch, offset_branch, rel_offsets, bias)
            )
...
class DSwinTransformerBlock3D(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        self.ffn = FeedForward3D(dim, expansion_factor=ffn_expansion_factor, bias=qkv_bias)
...
class DSwinEncoder3D(nn.Module):
...
class TransMorphDSwin3D(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.encoder = DSwinEncoder3D(config)
...
CONFIGS = {
    'TransMorph-DSwin3D': configs.get_3DTransMorphDSwin_config(),
    'TransMorph-DSwin3D-Tiny': configs.get_3DTransMorphDSwinTiny_config(),
}
